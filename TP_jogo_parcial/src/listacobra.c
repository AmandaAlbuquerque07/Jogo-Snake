...
void IniciaSnake(Jogo *j){
    ...
    j->snake.Cabeca->body.pos = (Rectangle){ j->LARGURA/2 - STD_SIZE_X -10, j->ALTURA - STD_SIZE_Y - 10, STD_SIZE_X, STD_SIZE_Y };
...
void IniciaBordas(Jogo *j){
    //Borda de cima
    j->bordas[0].pos = (Rectangle) {0, 0, j->LARGURA, (10*j->escala)};
    //Borda da direita
    j->bordas[1].pos = (Rectangle) {(j->LARGURA - 10*j->escala), 0, (10*j->escala), j->ALTURA};
    //Borda de baixo
    j->bordas[2].pos = (Rectangle) {0, (j->ALTURA - 10*j->escala), j->LARGURA, (10*j->escala)};
    //Borda da esquerda
    j->bordas[3].pos = (Rectangle) {0, 0, (10*j->escala), j->ALTURA};
}
